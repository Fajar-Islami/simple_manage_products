...
func DatabaseInit(v *viper.Viper) *gorm.DB {
	...
	mysqldb, err := db.DB()
	if err != nil {
		helper.Logger(currentfilepath, helper.LoggerLevelPanic, "", fmt.Errorf("Cannot conenct to database : %s", err.Error()))
		panic(err)
	}
...
func CloseDatabaseConnection(db *gorm.DB) {
	dbSQL, err := db.DB()
	if err != nil {
		helper.Logger(currentfilepath, helper.LoggerLevelPanic, "", fmt.Errorf("Failed to close connection to database : %s", err.Error()))
	}
